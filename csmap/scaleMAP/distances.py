...
# Note: hierarchical_categorical_distance is intentionally NOT decorated with @numba.njit().
# The cat_hierarchy argument is a list-of-dicts (a Python object), which numba cannot reliably
# reflect in nopython mode across all versions. Removing the default and applying @numba.njit()
# was attempted; however, list-of-dicts reflection in numba is fragile and version-dependent.
# This function is only called during graph construction (not the SGD hot path), so pure-Python
# performance is acceptable. All call sites in umap_.py pass cat_hierarchy explicitly.
