...
def _optimize_layout_euclidean_single_epoch_serial(
    head_embedding,
    tail_embedding,
    head,
    tail,
    ...
    # dens_R_gamma = dens_R**(1.0/2.0)
    # dens_R_gamma = dens_R**0.5
    dens_R_gamma = dens_R
    # r_max = np.max(dens_R_gamma)/2             
    r_max = np.percentile(dens_R_gamma, 95)
    # r_max = np.max(dens_R_gamma)
    # f_arr = np.abs((alpha-0.5)*2.0) + (alpha-0.5)*2.0 + beta*(dens_R_gamma/r_max)    
    # f_arr = 1.0 + 0.0*(dens_R_gamma/r_max)
    # frac_epochs = 1-alpha
    ...
    # elif frac_epochs < 0.75:
    #     umap_frac = (3-4*frac_epochs) # Make it go to 0 from 0.5 to 0.75
    #     scale_frac = 1 - umap_frac
    # elif frac_epochs <= 1.0:
    #     umap_frac = 0.0
    #     scale_frac = 1.0

    # f_arr = np.ones_like(dens_R_gamma)*umap_frac + (dens_R_gamma/r_max)*scale_frac

    f_arr = (dens_R_gamma/r_max)
    ...
    for i in range(epochs_per_sample.shape[0]):
        if epoch_of_next_sample[i] <= n:
            ...
            if densmap_flag:
                ...
                dist_squared = rdist(current, other)/f_arr[j]**2
                # dist_squared_curr = rdist(current, other)/f_arr[j]**2
                # dist_squared_oth = rdist(current, other)/f_arr[j]**2
                ...
                for d in range(dim):
                    ...
                    # grad_d = clip(grad_coeff * (current[d] - other[d])/f_arr[k])
                    # grad_d = clip(grad_coeff * (current[d] - other[d]))/f_arr[j]
                    grad_d = scaled_clip(grad_coeff * (current[d] - other[d])/f_arr[j], f_arr[j])
                ...
                if move_other:
                    dist_squared = rdist(current, other)/f_arr[k]**2
                    # dist_squared_curr = rdist(current, other)/f_arr[j]**2
                    # dist_squared_oth = rdist(current, other)/f_arr[j]**2
                    ...
                    for d in range(dim):
                        # grad_d = clip(grad_coeff * (current[d] - other[d]))
                        # grad_d = clip(grad_coeff * (current[d] - other[d])/f_arr[j])
                        grad_d = scaled_clip(grad_coeff * (current[d] - other[d])/f_arr[k], f_arr[k])
                        # grad_d = clip(grad_coeff * (current[d] - other[d]))/f_arr[j] 
                ...
                for p in range(n_neg_samples):
                    ...
                    # dist_squared = rdist(current, other)/f**2
                    dist_squared = rdist(current, other)/f_arr[j]**2

                    if dist_squared > 0.0:
                        grad_coeff = 2.0 * gamma * b
                        # grad_coeff = 2.0 * gamma * b * 10.0
                    ...
                    for d in range(dim):
                        if grad_coeff > 0.0:
                            # grad_d = clip(grad_coeff * (current[d] - other[d]))
                            # grad_d = clip(grad_coeff * (current[d] - other[d])/f)
                            # grad_d = clip(grad_coeff * (current[d] - other[d])/f_arr[k])
                            # grad_d = clip(grad_coeff * (current[d] - other[d]))/f_arr[j]
                            grad_d = scaled_clip(grad_coeff * (current[d] - other[d])/f_arr[j], f_arr[j])
                        else:
                            ...
                            # grad_d = 4.0/f_arr[k]
                        current[d] += grad_d * alpha
                        # # current[d] += grad_d * (100.0**1) * alpha
                ...
                if dist_squared > 0.0:
                    # grad_coeff = -2.0 * a * b * pow(dist_squared, b - 1.0) 
                    # grad_coeff = -2.0 * a * b * pow(dist_squared, b - 1.0) * 10.0
                    # grad_coeff /= a * pow(dist_squared, b) + 1.0
                    grad_coeff = -2.0 * a * b * pow(dist_squared, b - 1.0) 
                    grad_coeff /= a * pow(dist_squared, b) + 1.0
                else:
                ...
                for d in range(dim):
                    ...
                    current[d] += grad_d * alpha
                    # current[d] += grad_d * alpha * 0.0
                    if move_other:
                        other[d] += -grad_d * alpha
                        # other[d] += -grad_d * alpha * 0.0
...
def _optimize_layout_euclidean_single_epoch_parallel(
    head_embedding,
    tail_embedding,
    head,
    tail,
...
@numba.njit(fastmath=True)
def _optimize_layout_euclidean_densmap_epoch_init_serial(
    head_embedding,
    tail_embedding,
    head,
    tail,
...
@numba.njit(fastmath=True, parallel=True)
def _optimize_layout_euclidean_densmap_epoch_init_parallel(
    head_embedding,
    tail_embedding,
    head,
    tail,
...
def optimize_layout_euclidean(
    ...
    optimize_fn = (
        _optimize_layout_euclidean_single_epoch_parallel if parallel
        else _optimize_layout_euclidean_single_epoch_serial
    )
    ...
    if densmap:
        dens_init_fn = (
            _optimize_layout_euclidean_densmap_epoch_init_parallel if parallel
            else _optimize_layout_euclidean_densmap_epoch_init_serial
        )
    ...
    for n in tqdm(range(n_epochs), **tqdm_kwds):
        ...
        # alpha = initial_alpha * np.max([(1.0 - 2.0*(float(n) / float(n_epochs))),0.0])
        # if n >= n_epochs // 2:
        #     beta = 2.0 * (1.0 - (float(n) / float(n_epochs)))

        if n <= n_epochs // 2:
            beta = 2.0 * (float(n) / float(n_epochs))
        else:
            beta = 2.0 * (1.0 - (float(n) / float(n_epochs)))

        embedding_history.append(head_embedding.copy())

    return head_embedding, embedding_history
