...
def init_outside_crikit(xlen_in, ylen_in, zlen_in):
	global yi, xi, xlen, ylen, zlen
	xlen = xlen_in
	ylen = ylen_in
	zlen = zlen_in
...
def standardize_get_zscaler(img,mask):
	# if (mask is None):
...
def import_submasks(maskfolderpath=None):
	if (maskfolderpath is None):
	...
	for file in maskfiles:
		...
		img = plt.imread(file)[:,:]
		masks.append(img[:,:,3] > 0) # Select pixels with positive alpha as "in" the mask
...
def import_main_mask(mainmaskpath=None):
	if (mainmaskpath is None):
	...
	mainmask = (img[:,:,3] > 0) # Select pixels with positive alpha as "in" the mask
...
def import_stdimg_pickle(path=None):
	if (path is None):
...
def import_nrmimg_pickle(path=None):
	if (path is None):
...
def import_pca_pickle(path=None):
	if (path is None):
...
def import_scaler_pickle(path=None):
	if (path is None):
...
def export_named_np_array_to_FCS(labels,array,filename):
	with open(filename + '.fcs', 'wb') as file_handle:
		flowio.create_fcs(	file_handle,
							array.flatten(),
							labels)
...
def updateSpadeMaskFromCSV(maskpath, color):
	try:
...
def updateSpadeMask(maskpath, color):
	try:
		# mask = plt.imread(maskpath) !=0
		mask = np.array(Image.open(maskpath)) !=0
	except:
		print("File was unpopulated")
		return
	spade_out = {'mask' : np.flipud(mask), 'color' : color}
	if (len(crikit_data.img_overlays) > 0) : 
		crikit_data.img_overlays.pop()
...
def updateSpadeMaskContinuously(color=[0,255,0,255]):
	...
	commsMaskPath = cmdgetfilepath("Please drag and drop the PNG comm file MATLAB is updating: ")
	...
	input("Press any key to stop listening for updates to the PNG comm file")
