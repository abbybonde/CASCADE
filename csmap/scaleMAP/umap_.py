...
import scaleMAP.distances as dist

import scaleMAP.sparse as sparse

from scaleMAP.utils import (
    submatrix,
    ts,
    csr_unique,
    fast_knn_indices,
)
from scaleMAP.spectral import spectral_layout
from scaleMAP.layouts import (
...
def simplicial_set_embedding(
    ...
    if densmap or output_dens:
        ...
        for i in range(len(head)):
            ...
            # D = (dists[j, k] * dists[j, k])**0.5  # match sq-Euclidean used for embedding
            mu = graph.data[i]
    ...
    if euclidean_output:
        embedding, embedding_history = optimize_layout_euclidean(
            embedding,
            embedding,
            head,
            tail,
    ...
    if output_dens:
        ...
        re = epsilon + (re / mu_sum)

        aux_data["rad_emb"] = re
        aux_data["embedding_history"] = embedding_history
    ...
    def fit(self, X, y=None):
        ...
        if self.transform_mode == "embedding":
            ...
            if self.output_dens:
                ...
                self.emb_history_ = aux_data["embedding_history"]
