...
def process_conv_deriv_fit(
    ...
    sigmas: torch.Tensor,
    gammas: torch.Tensor,
    wavelet_peaks: torch.Tensor = None,  # only used for 'voigt'; ignored for 'Lor4' (built internally)
    response_threshold: float = 0.0001,
    ...
    """
    device   = wavelet_peaks.device if wavelet_peaks is not None else None
    spectrum = _to_tensor(spectrum, device)
    x        = _to_tensor(x,        device)
    sigmas   = _to_tensor(sigmas,   device)
    gammas   = _to_tensor(gammas,   device)
...
def _fit_batch_adam(
    ...
    # --- Precompute LR schedule (avoids math.cos every step) ---
    ...
    optimizer = torch.optim.Adam([params], lr=peak_lr, betas=(0.9, 0.999), fused=True)
    pg = optimizer.param_groups[0]  # only one group — unroll the loop
    ...
    for i in range(max_iter):
        ...
        with torch.no_grad():
            ...
            # Fused mask + clip in one multiply
            norms    = g.norm(dim=1, keepdim=True)     # (B, 1)
            clip_now = aggressive_clip_norm if i < aggressive_steps else 1.0
            scale    = clip_now / norms.clamp(min=clip_now)   # ≤ 1.0
            # Only pay for the mul_ if any row actually exceeds the clip threshold
            if (norms > clip_now).any():
                params.grad.mul_(grad_mask * scale)
            else:
                params.grad.mul_(grad_mask)

        # Update LR and betas from precomputed schedule — no Python math
        pg['lr']    = lrs[i].item()
        pg['betas'] = (betas1[i].item(), 0.999)

        optimizer.step()

        # Bounds projection — cheap in-place clamp most steps,
        # full NaN-safe version only every 10 steps
        with torch.no_grad():
            if i % 10 == 0:
                p_r     = params.data.reshape(B, -1, 4)
                clamped = torch.clamp(p_r, _LO, _HI)
        ...
        if progress_every and progress_every > 0:
            ...
            if step_done % progress_every == 0 or step_done == max_iter:
                ...
                print(
                    f"[{progress_prefix}] iter {step_done}/{max_iter} "
                    f"({100.0 * step_done / max_iter:.1f}%) | "
                    f"loss/pix={loss.item():.4e} | lr={pg['lr']:.2e} | "
                    f"elapsed {elapsed:.1f}s | ETA {eta:.1f}s"
                )
...
# ── Normalisation helpers (moved from SyntheticData notebook) ────────────────

def _normalize_noise_levels(levels) -> List[float]:
    """Cast an iterable of noise level values to a list of Python floats."""
    return [float(n) for n in levels]


def _normalize_sep_levels(levels) -> List[tuple]:
    """Cast an iterable of separability range values to a list of tuples."""
...
def _atomic_save(obj: dict, path: str) -> None:
    ...
    into place.  Because rename(2) is atomic on POSIX (and near-atomic on
    Windows with os.replace), the old checkpoint is never overwritten until
    the new one is fully flushed to disk.  A crash during the write leaves
    the previous checkpoint intact rather than a zero-byte stub.
    """
    dir_ = os.path.dirname(os.path.abspath(path)) or "."
    # Use the same filesystem as the target so rename stays atomic
    with tempfile.NamedTemporaryFile(delete=False, dir=dir_, suffix=".tmp") as tmp:
        tmp_path = tmp.name
    try:
        torch.save(obj, tmp_path)
        os.replace(tmp_path, path)          # atomic on POSIX; os.replace on Win
    except Exception:
...
def run_batch_fit(
    ...
    Each batch is written to its own ``ckpt_{name}/batch_{start:06d}_{end:06d}.pt``
    file immediately after processing.  The progress checkpoint (``ckpt_{name}/progress.pt``)
    stores only metadata (no data arrays), so RAM usage stays flat regardless of N_TOTAL.
    On successful completion the entire ``ckpt_{name}/`` folder is deleted.

    On resume, completed batches are detected from existing batch files rather
    than loading a monolithic checkpoint array.

    Requires ``import shutil, glob`` in addition to the usual imports.
    ...
    Returns
    -------
    dict with keys: params_all, model_all, spec_d, condition_labels, x_axis,
                    f1_vals, prec_vals, rec_vals, all_stats
    """
    ...
    x_np            = _ctx_x.copy()
    ckpt_dir        = f"ckpt_{name}"
    checkpoint_path = os.path.join(ckpt_dir, "progress.pt")
    os.makedirs(ckpt_dir, exist_ok=True)

    # all_stats is the only thing kept in memory — it's a list of small dicts
    all_stats = [None] * N_TOTAL

    # On resume, reload all_stats from the progress checkpoint (metadata only)
    resume_from = 0
    if resume:
    ...
    try:
        for start in range(resume_from, N_TOTAL, batch_size):
            ...
            # Skip batches whose output file already exists (safe re-resume)
            batch_path = os.path.join(ckpt_dir, f"batch_{start:06d}_{end:06d}.pt")
            if resume and os.path.exists(batch_path):
                print(f"[batch {start}:{end}] already done, skipping")
                continue
            ...
            if cur_cols < target_cols:
                pad           = torch.zeros(B, target_cols - cur_cols,
                                            dtype=params_pruned.dtype,
                                            device=params_pruned.device)
                params_pruned = torch.cat([params_pruned, pad], dim=1)
            elif cur_cols > target_cols:
                params_pruned = params_pruned[:, :target_cols]

            # 5) Reconstruct model from pruned params so model_all is consistent
            with torch.no_grad():
                model_batch = _compute_model_batch(params_pruned, _x_dev)

            n_fit_peaks = (params_pruned.reshape(B, -1, 4)[:, :, 0] > amp_threshold).sum(dim=1)

            _sync_device(_ctx_device)
            total_sec = time.perf_counter() - t0

            # 6) Write this batch to disk immediately — no accumulation in RAM
            _atomic_save({
                "params":      params_pruned.cpu(),
                "model":       model_batch.cpu(),
                "spec_d":      spec_d.cpu(),
                "start":       start,
                "end":         end,
            }, batch_path)
            ...
            # 7) Peak matching against ground-truth (skipped if no samples provided)
            if samples is not None:
                params_cpu = params_pruned.cpu()
                for i, i_global in enumerate(range(start, end)):
                    s = samples[i_global]
            ...
            # 8) Progress checkpoint — metadata only, no data arrays
            ...
            # 9) Free everything before next batch
            del params_fit, params_pruned, model_batch, p0_batch
            del flat_masks, masks, resp, spectra_gpu, spec_d
            del grad_mask, peak_masks, vote_count, p0_flat_list, n_real_peaks
            torch.cuda.empty_cache()

    except KeyboardInterrupt:
        print(f"\nInterrupted — progress saved to {checkpoint_path}. Re-run with resume=True.")
        # Stitch whatever completed batches exist and return partial results
        return _stitch_from_disk(ckpt_dir, all_stats, condition_labels, x_np)
    ...
    # ------------------------------------------------------------------ #
    # All batches done — stitch batch files and return                     #
    # ------------------------------------------------------------------ #
    _sync_device(_ctx_device)
    ...
    results = _stitch_from_disk(ckpt_dir, all_stats, condition_labels, x_np)
    ...
    # Clean up checkpoint folder now that results are saved
    shutil.rmtree(ckpt_dir)
    print(f"Removed checkpoint folder: {ckpt_dir}/")

    return results


def _stitch_from_disk(ckpt_dir, all_stats, condition_labels, x_np):
    """Load all batch_*.pt files from ckpt_dir and concatenate into a result dict."""
    batch_files = sorted(glob.glob(os.path.join(ckpt_dir, "batch_*.pt")))
    if not batch_files:
        raise RuntimeError(f"No batch files found in {ckpt_dir}/")
    ...
    for f in batch_files:
        ...
        model_list.append(ckpt["model"])
        spec_d_list.append(ckpt["spec_d"])
    ...
    return {
        "params_all":       torch.cat(params_list, dim=0),
        "model_all":        torch.cat(model_list,  dim=0),
        "spec_d":           torch.cat(spec_d_list, dim=0),
